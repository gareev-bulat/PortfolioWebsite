...
const ExperienceCard = ({ name, image, position, bullets }: CardProps) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="absolute inset-0 backface-hidden bg-brand-teal rounded-3xl border-[7px] border-brand-700 flex items-center justify-center gap-6 px-6">
          {!flipped && (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-sm text-brand-700/60 animate-bounce">
              Click to flip
            </div>
          )}
          <Image
